...
/**
 * Xiaomi Health Service Implementation.
 *
 * <p>Implements OAuth 2.0 authorization flow for Xiaomi Sport/Health platform.
...
@Slf4j
@Service
public class XiaomiHealthServiceImpl implements XiaomiHealthService, DevicePlatformService {

    private static final String PLATFORM = "xiaomi";
    private static final String STATE_KEY_PREFIX = "oauth:state:xiaomi:";
    ...
    public XiaomiHealthServiceImpl(
    ...
    public String getAuthorizationUrl(Long userId) {
        if (!isConfigured()) {
            log.warn("Xiaomi Health OAuth is not configured");
            return null;
        }

        PlatformConfig config = oAuthProperties.getPlatform(PLATFORM);
        if (config == null) {
            log.error("Xiaomi platform configuration not found");
            return null;
        }
        ...
        log.info("Generated Xiaomi Health OAuth URL for user {}", userId);
    ...
    public void handleCallback(String code, String state) {
        log.info("Processing Xiaomi Health OAuth callback");
        ...
        log.info("Xiaomi Health OAuth completed successfully for user {}", userId);
    }
    ...
    public boolean refreshTokens(Long userId) {
        if (!isConfigured()) {
            log.warn("Cannot refresh tokens: Xiaomi Health OAuth is not configured");
            return false;
        }
        ...
        if (connection == null || connection.getRefreshToken() == null) {
            log.warn("No Xiaomi connection found for user {}", userId);
            return false;
        }
    ...
    public SyncResultVO syncData(Long userId) {
        ...
        log.info("Starting Xiaomi Health data sync for user {}", userId);

        // Step 1: Check device connection status
        DeviceConnection connection = connectionService.getConnection(userId, PLATFORM);
        if (connection == null) {
            log.warn("No Xiaomi device connection found for user {}", userId);
            return syncHistoryService.createFailedRecord(
                    userId, PLATFORM, "manual",
                    "Device not connected. Please connect your Xiaomi Health account first.",
                    (int) (System.currentTimeMillis() - startTime)
            );
        }

        if (!"connected".equals(connection.getStatus())) {
            log.warn("Xiaomi device connection is not active for user {}: {}", userId, connection.getStatus());
            return syncHistoryService.createFailedRecord(
                    userId, PLATFORM, "manual",
        ...
        try {
            ...
            if (accessToken == null) {
                ...
                return syncHistoryService.createFailedRecord(
                        userId, PLATFORM, "manual",
                        "Failed to authenticate with Xiaomi Health. Please reconnect your account.",
                        (int) (System.currentTimeMillis() - startTime)
                );
            }
        } catch (Exception e) {
        ...
        // Step 3: Fetch health data (with mock fallback)
        ...
        if (savedCount > 0) {
            log.info("Xiaomi Health sync completed for user {}: {} metrics saved", userId, savedCount);
            ...
            log.warn("Xiaomi Health sync completed for user {} with no data saved", userId);
            return syncHistoryService.createFailedRecord(
                    userId, PLATFORM, "manual",
    ...
    /**
     * Fetch health data with mock data fallback.
     */
    private List<HealthDataPoint> fetchHealthData(String accessToken, Long userId) {
        ...
        if (isConfigured()) {
            try {
                ...
                log.info("No data returned from Xiaomi API");
            } catch (Exception e) {
        ...
        log.error("Xiaomi API unavailable and mock data is disabled");
        throw new SystemException(ErrorCode.DEVICE_SYNC_FAILED,
                "无法获取小米运动数据。API不可用且模拟数据已禁用，请检查API配置或联系管理员。");
    }

    /**
     * Fetch real health data from Xiaomi API.
     * Note: Actual Xiaomi Health API endpoints need to be verified from official documentation.
     */
    private List<HealthDataPoint> fetchRealHealthData(String accessToken, Long userId) {
        // TODO: Implement real Xiaomi Health API integration
        // Xiaomi Sport API endpoints are not publicly documented
        // This placeholder returns empty list to trigger mock data fallback
        log.info("Xiaomi Health API integration not yet implemented for user {}", userId);
    ...
    private List<HealthDataPoint> generateMockHealthData() {
        ...
        mockData.add(new HealthDataPoint(
                "steps",
                BigDecimal.valueOf(5000 + random.nextInt(10000)),
                "steps",
                now,
                "step_count",
                PLATFORM
        ...
        // Sleep duration
        double sleepHours = 5 + random.nextDouble() * 4;
        mockData.add(new HealthDataPoint(
                "sleepDuration",
        ...
        mockData.add(new HealthDataPoint(
                "systolicBP",
                BigDecimal.valueOf(110 + random.nextInt(30)),
                "mmHg",
                now.minusHours(2),
                "blood_pressure",
                PLATFORM
        ...
        mockData.add(new HealthDataPoint(
                "diastolicBP",
                BigDecimal.valueOf(70 + random.nextInt(20)),
                "mmHg",
                now.minusHours(2),
                "blood_pressure",
                PLATFORM
        ...
        mockData.add(new HealthDataPoint(
                "glucose",
                BigDecimal.valueOf(80 + random.nextInt(60)),
                "mg/dL",
                now.minusHours(4),
                "blood_glucose",
                PLATFORM
